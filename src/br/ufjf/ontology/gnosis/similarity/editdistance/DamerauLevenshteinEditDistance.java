/**
 *A distancia Levenshtein-Damerau é determinada pela contagem mínima do
 *número de operações necessárias para transformar uma string em outra,
 *as operações são, a inserção, a exclusão, a substituição ou a transposição
 *de dois caracteres adjacentes.
 * Quando mais proximo de 1 elas são iguais.
 */
 					...
 					dp[i][j] = dp[i - 1][j - 1];
					    ...
					    dp[i][j] = Math.min(dp[i][j - 1] + 1, Math.min(dp[i - 1][j - 1] + 1, dp[i - 1][j] + 1));
                                            if ((i > 1) && (j > 1) && (s1.charAt(i - 1) == s2.charAt(j - 2)) && (s1.charAt(i - 2) == s2.charAt(j - 1)))
                                                dp[i][j] = Math.min(dp[i][j], dp[i - 2][j - 2] + 1); //Transposição
                                        }
                        }
